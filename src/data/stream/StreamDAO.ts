...
import { SetStreamDB, STREAM_NAME_OBJECT } from "../../domain/entity/stream/StreamConst"
...
export default class StreamDAO implements StreamRepository, CrudRepository<Stream> {
    ...
    find(idOpen: string): Promise<Stream | null> {
        return new Promise(async (resolve, reject) => {
    ...
    create(nameValue: string, userRegister: string, reviewedValue: boolean, createdAtValue: string): Promise<string> {
        return new Promise(async (resolve, reject) => {
            const objectDb = SetStreamDB(nameValue, (new ObjectId(userRegister)), reviewedValue, true, createdAtValue)
