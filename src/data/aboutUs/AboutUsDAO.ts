...
import { AboutUsSetObjectDB, ABOUT_US_NAME_OBJECT } from '../../domain/entity/aboutUs/AboutUsConst'
...
export default class AboutUsDAO implements AboutUsRepository, CrudRepository<AboutUs> {
    ...
    openById(idOpen: string): Promise<AboutUs | null> {
        throw new Error('Method not implemented.')
    }

    create(app: string, web: string): Promise<string> {
        return new Promise(async (resolve, reject) => {
            const objectDb = AboutUsSetObjectDB(app, web, ConvertData.getDateNowStr())
