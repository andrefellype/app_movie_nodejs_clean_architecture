...
import { SetMyMovieDB, MY_MOVIES_NAME_OBJECT } from "../../domain/entity/myMovie/MyMovieConst"
...
export default class MyMovieDAO implements MyMovieRepository, CrudRepository<MyMovie> {
    ...
    find(idOpen: string): Promise<MyMovie> {
        throw new Error("Method not implemented.")
    }

    create(userId: string, movieId: string, createdAtValue: string): Promise<string> {
        return new Promise(async (resolve, reject) => {
            const objectDb = SetMyMovieDB(new ObjectId(userId), new ObjectId(movieId), createdAtValue)
