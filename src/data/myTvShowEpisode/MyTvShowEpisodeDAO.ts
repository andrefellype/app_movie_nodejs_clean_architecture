...
import { SetMyTvShowEpisodeDB, MY_TV_SHOW_EPISODES_NAME_OBJECT } from "../../domain/entity/myTvShowEpisode/MyTvShowEpisodeConst"
...
export default class MyTvShowEpisodeDAO implements MyTvShowEpisodeRepository, CrudRepository<MyTvShowEpisode> {
    ...
    find(idOpen: string): Promise<MyTvShowEpisode> {
        throw new Error("Method not implemented.")
    }

    create(userId: string, tvShowId: string, tvShowSeasonId: string, tvShowEpisodeId: string, createdAtValue: string): Promise<string> {
        return new Promise(async (resolve, reject) => {
            const objectDb = SetMyTvShowEpisodeDB(new ObjectId(userId), new ObjectId(tvShowId), (new ObjectId(tvShowSeasonId)), (new ObjectId(tvShowEpisodeId)), createdAtValue)
    ...
    findAll(): Promise<MyTvShowEpisode[]> {
        throw new Error("Method not implemented.")
    }
}